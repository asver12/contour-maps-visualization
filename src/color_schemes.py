...
def find_start_color(color):
    pass


def find_end_color(color):
    pass


def find_perentage(index, start_index, end_index):
    pass
...
def create_color_brewer_colorscheme(colorscheme_name, levels, min_value=0, max_value=1, lvl_white=1, verbose=False):
    ...
    colorscheme = colorschemes.get(colorscheme_name, blue_color_scheme)
    norm_levels = np.linspace(min_value, max_value, len(levels) + 1)
    for i in norm_levels:
        start_index, start_color = find_start_color(i)
        end_index, end_color = find_end_color(i)
        if end_color != start_color:
            interpolate(start_color, end_color, find_perentage(i, start_index, end_index))

def interpolate(start_color, end_color, percentage):
    return [col_1 * percentage + col_2 * (1 - percentage) for col_1, col_2 in zip(start_color, end_color)]
...
def linear_interpolation_between_two_colors():
    """
    247 251 255 |   58  100 98
    222 235 247 |   58  61  92
    198 219 239 |   58  56  86
    158 202 225 |   56  53  75
    8   48  107 |   60  86  23
